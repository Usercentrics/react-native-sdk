...
class RNUsercentricsModuleTests: XCTestCase {
  ...
  func testIsReadyWithError() {
    ...
    module.isReady { _ in
      ...
      XCTAssertEqual("The operation couldn’t be completed. (sampleTests.FakeUsercentricsError error 0.)", message)
    }
  }
  ...
  func testDenyAllForTCF() {
    ...
    module.denyAllForTCF(0, consentType: 0, unsavedPurposeLIDecisions: [], unsavedVendorLIDecisions: []) { [unowned self] result in
      guard
        let result = result as? [NSDictionary],
        let consent = result.first
      else {
  ...
  func testDenyAllForTCFSecondLayer() {
    fakeUsercentrics.denyAllForTCFResponse = [.mock()]
    module.denyAllForTCF(1, consentType: 1, unsavedPurposeLIDecisions: [], unsavedVendorLIDecisions: []) { [unowned self] _ in
